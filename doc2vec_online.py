...
class DocvecsArray(utils.SaveLoad):
    ...
    def note_newdoctag(self, key, document_no, document_length, model):
        print 'syn'
        # print self.doctag_syn0
        # print self.doctags[key]
        # print len(self.offset2doctag)
        # self.doctag_syn0[self.doctags[key].offset-2] = model.seeded_vector(model.seed)
    ...
    def note_doctag(self, key, document_no, document_length, model):
        """Note a document tag during initial corpus scan, for structure sizing."""
        # print len(self.doctags)
    ...
    def update_weights(self, model):
        ...
        # self.doctag_syn0 = ones(len(self.doctags), dtype=REAL)  # zeros suppress learning
        # self.doctag_syn0 = empty((len(self.doctags), model.vector_size), dtype=REAL)
...
class Doc2Vec(Word2Vec):
    ...
    def build_vocab(self, documents, keep_raw_vocab=False, trim_rule=None, progress_per=10000, update=False):
        """
        Build vocabulary from a sequence of sentences (can be a once-only generator stream).
        Each sentence must be a list of unicode strings.
        """
        self.scan_vocab(documents, progress_per=progress_per, trim_rule=trim_rule, update=update)  # initial survey
    ...
    def scan_vocab(self, documents, progress_per=10000, trim_rule=None, update=False):
        ...
        new_tags_total = pre_exist_tags_total = 0
        new_tags = pre_exist_tags = []
