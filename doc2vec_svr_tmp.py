...
def regression(corpus_doc,corpus_doc2):
	#グローバル変数とローカル変数の混在を防ぐための宣言
	global model
	#各変数の初期化、構造定義
	X = np.empty((0,len(model.docvecs[0])), float)
	y = np.empty((0,1), float)
	test_X = np.empty((0,len(model.docvecs[0])), float)
	...
	for x in range(len(corpus_doc)):
		...
		#深層表現を格納
		#X = np.asarray(model.docvecs[x])
		#X = np.append(X,model.docvecs[x])[:, np.newaxis]
		X = np.append(X, np.array([model.docvecs[x]]), axis=0)
		#ROAを格納
		#y = np.asarray([float(roa)])
		#y = np.append(y,float(roa))[:, np.newaxis]
	...
	for j in range(100):
		#print(j)
		#test_X = np.append(test_X, np.array([model.infer_vector(docs2[j])]), axis=0)
		#深層表現を格納
		test_X = np.append(test_X, np.array([model.infer_vector(sentences2[j].words)]), axis=0)
